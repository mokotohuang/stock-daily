...
function dataCard(label, value, sub, valueTone = "") {
  return `<article class="data-card"><p>${label}</p><strong class="${valueTone}">${value}</strong><small>${sub}</small></article>`;
}
...
function render(data) {
  ...
  byId("tw-cards").innerHTML = [
    dataCard("加權指數", number(m.close, 2), `${signed(m.change, " 點")}｜${signed(m.changePct, "%")}`, tone(m.changePct)),
    dataCard("成交金額", number(m.turnover), "上市股票成交金額"),
    dataCard("外資", signed(i.foreign, " 億"), "買賣超金額", tone(i.foreign)),
    dataCard("投信", signed(i.trust, " 億"), "買賣超金額", tone(i.trust)),
    dataCard("自營商", signed(i.dealer, " 億"), "自行買賣＋避險", tone(i.dealer)),
    dataCard("融資餘額", `${number(mg.balance)} 億`, `今日變化 ${signed(mg.change, " 億")}`, tone(-mg.change))
  ].join("");
  byId("breadth").innerHTML = `上漲 <b>${m.advances}</b> 家　下跌 <b>${m.declines}</b> 家　跌停 <b>${m.limitDown}</b> 家`;
  byId("us-cards").innerHTML = data.indices.map((item) => `<article class="index-card"><div><span class="market-dot ${item.change == null ? "flat" : item.change >= 0 ? "up" : "down"}"></span><b>${item.name}</b><em>${item.code}</em></div><strong>${number(item.close, 2)}</strong><p class="${tone(item.change)}">${item.change == null ? "—" : `${signed(item.pointChange, " 點")}｜${signed(item.change, "%")}`}</p><small>${item.note}</small></article>`).join("");
}
